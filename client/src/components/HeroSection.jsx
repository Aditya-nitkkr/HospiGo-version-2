...
export const HeroSection = () => {
    ...
    const handleNearMe = () => {
        ...
        navigator.geolocation.getCurrentPosition(
            (position) => {
                ...
                // If accuracy is worse than ~2 km, warn the user or prompt manual entry
                if (accuracy > 2000) {
                    toast.warn("Location accuracy is low. Please verify your city.");
                }
            ...
            (error) => {
                ...
                switch (error.code) {
                    ...
                    case error.POSITION_UNAVAILABLE:
                        ...
                        break;
                    case error.TIMEOUT:
                        toast.error("Request to get your location timed out.");
                        break;
                    default:
    ...
    return (
        <section className="relative bg-gradient-to-b from-indigo-50/50 to-white pt-12 pb-20 lg:pt-20 lg:pb-28">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-8 items-center">
                    ...
                    <div className="flex flex-col items-start text-left">
                        ...
                        {/* Search Bar Wrapper with elevated z-index & outside-click ref */}
                        <div
                            ref={searchContainerRef}
                            className="relative w-full max-w-2xl mb-8 z-30"
                        >
                            <div className="flex flex-col sm:flex-row items-center bg-white p-2 rounded-2xl sm:rounded-full shadow-lg border border-gray-200 focus-within:ring-2 focus-within:ring-indigo-500 transition-all duration-300">
                                ...
                                <div className="flex w-full sm:w-auto gap-2 mt-2 sm:mt-0 px-2 sm:px-0 pb-2 sm:pb-0">
                                    <button
                                        ...
                                        disabled={isLocating}
                                        className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-6 py-3 bg-indigo-50 text-indigo-700 font-semibold rounded-xl sm:rounded-full hover:bg-indigo-100 transition-colors whitespace-nowrap disabled:opacity-60 disabled:cursor-not-allowed"
                                    >
                                        <IoLocationOutline className={`text-lg ${isLocating ? "animate-spin text-indigo-600" : ""}`} />
                            ...
                            {showDropdown && suggestions.length > 0 && (
                                <ul className="absolute top-[calc(100%+8px)] left-0 right-0 bg-white border border-gray-200 rounded-2xl shadow-2xl max-h-64 overflow-y-auto z-50 divide-y divide-gray-100">
                                    {suggestions.map((s, idx) => (
                                        ...
                                        >
                                            <IoLocationOutline className="text-indigo-500 flex-shrink-0 text-lg" />
                        ...
                        {/* CTA Button */}
                        {!isAuthenticated && (
                            <NavLink to="/signup" className="relative z-10">
                                <button
                                    type="button"
